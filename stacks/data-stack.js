import { Bucket, Config, Table } from 'sst/constructs'
...
export function DataStack ({ stack, app }) {
  ...
  // Secrets
  const aggregatorPrivateKey = new Config.Secret(
    stack,
    'AGGREGATOR_PRIVATE_KEY'
  )
  const dealerPrivateKey = new Config.Secret(stack, 'DEALER_PRIVATE_KEY')
  const dealTrackerPrivateKey = new Config.Secret(
    stack,
   ...
   */
  const aggregatorBufferBucket = getBucketConfig(
    'aggregator-buffer-store',
    stack.stage
  )
  const aggregatorBufferStoreBucket = new Bucket(
    stack,
    aggregatorBufferBucket.bucketName,
    {
      cors: true,
      cdk: {
  ...
  const aggregatorPieceStoreTable = new Table(
    stack,
    aggregatorPieceStoreTableName,
    {
      ...aggregatorPieceStoreTableProps,
  ...
  const aggregatorAggregateStoreTable = new Table(
    stack,
  ...
  const aggregatorInclusionStoreTable = new Table(
    stack,
    aggregatorInclusionStoreTableName,
    {
      ...aggregatorInclusionStoreTableProps,
      // information that will be written to the stream
      stream: 'new_and_old_images'
   ...
   * to create an aggregate with desired size.
   */
  const aggregatorInclusionProofBucket = getBucketConfig(
    'aggregator-inclusion-proof-store',
    stack.stage
  )
  const aggregatorInclusionProofStoreBucket = new Bucket(
    stack,
    aggregatorInclusionProofBucket.bucketName,
    {
      cors: true,
      cdk: {
  ...
  const dealerOfferStoreBucket = new Bucket(
    stack,
    dealerOfferBucket.bucketName,
    {
      cors: true,
      cdk: {
  ...
  const dealerAggregateStoreTable = new Table(
    stack,
   ...
   */
  const dealTrackerDealArchiveBucket = getBucketConfig(
    'deal-tracker-deal-archive-store',
    stack.stage,
    1
  )
  const dealTrackerDealArchiveStoreBucket = new Bucket(
    stack,
    dealTrackerDealArchiveBucket.bucketName,
    {
      cors: true,
      cdk: {
  ...
  const dealTrackerDealStoreTable = new Table(
    stack,
    dealTrackerDealStoreTableName,
    {
      ...dealStoreTableProps,
      // information that will be written to the stream
      stream: 'new_and_old_images'
