...
export function ApiStack ({ app, stack }) {
  const { AGGREGATOR_HOSTED_ZONE, AGGREGATOR_DID, UCAN_LOG_URL } =
    getAggregatorEnv(stack)
  const { DEAL_TRACKER_API_HOSTED_ZONE, DEAL_TRACKER_DID, DEAL_TRACKER_PROOF } =
    getDealTrackerEnv()
  const { DEAL_API_HOSTED_ZONE, DEALER_DID } = getDealerEnv()
  const {
  ...
  const { pieceQueue, bufferQueue, aggregateOfferQueue, pieceAcceptQueue } =
    use(AggregatorStack)
  const pkg = getApiPackageJson()
  const git = getGitInfo()
  const ucanLogBasicAuth = new Config.Secret(stack, 'UCAN_LOG_BASIC_AUTH')
  const dealTrackerApiCustomDomain = getCustomDomain(
    stack.stage,
  ...
  const api = new Api(stack, 'aggregator-api', {
    ...
    routes: {
      ...
      'POST /':
        'packages/functions/src/aggregator-api/ucan-invocation-router.handler'
    }
  })
    ...
    defaults: {
      function: {
        environment: {
          ...
          SERVICE_URL: dealTrackerApiCustomDomain?.domainName
            ? `https://${dealTrackerApiCustomDomain?.domainName}`
            : '',
        ...
        bind: [dealerPrivateKey, dealerAggregateStoreTable, ucanLogBasicAuth],
        permissions: [dealerOfferStoreBucket]
      }
    },
    routes: {
      ...
      'POST /':
        'packages/functions/src/dealer-api/ucan-invocation-router.handler'
    }
  })
  ...
  const dealTrackerApi = new Api(stack, 'deal-tracker-api', {
    ...
    routes: {
      ...
      'POST /':
        'packages/functions/src/deal-tracker-api/ucan-invocation-router.handler'
    }
  })
  ...
  if (stack.stage === 'prod') {
    const dealMonitorAlertCronName = getResourceName(
      'deal-monitor-alert-cron',
      stack.stage
    )
    new Cron(stack, dealMonitorAlertCronName, {
      ...
      job: {
        function: {
          timeout: '5 minutes',
          handler:
            'packages/functions/src/monitor/handle-deal-monitor-alert-cron-tick.main',
          environment: {
            MIN_PIECE_CRITICAL_THRESHOLD_MS,
            MIN_PIECE_WARN_THRESHOLD_MS,
            AGGREGATE_MONITOR_THRESHOLD_MS,
          ...
          bind: [dealerAggregateStoreTable, aggregatorAggregateStoreTable]
        }
      }
    })
  }
  ...
  stack.addOutputs({
    ...
    AggregatorApiCustomDomain: aggregatorApiCustomDomain
      ? `https://${aggregatorApiCustomDomain.domainName}`
      : 'Set AGGREGATOR_HOSTED_ZONE in env to deploy to a custom domain',
    DealTrackerApiEndpoint: dealTrackerApi.url,
    DealTrackerApiCustomDomain: dealTrackerApiCustomDomain
      ? `https://${dealTrackerApiCustomDomain.domainName}`
      : 'Set DEAL_TRACKER_API_HOSTED_ZONE in env to deploy to a custom domain',
    DealerApiEndpoint: dealerApi.url,
    DealerApiCustomDomain: dealerApiCustomDomain
      ? `https://${dealerApiCustomDomain.domainName}`
      : 'Set DEALER_API_HOSTED_ZONE in env to deploy to a custom domain'
  })
