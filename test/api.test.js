...
import { randomCargo } from '@storacha/filecoin-api/test'
...
test('GET /version', async (t) => {
  ...
  // `aggregator-api`
  const aggregatorApiResponse = await fetch(
    `${t.context.api.aggregator}/version`
  )
  ...
  // `deal-tracker-api`
  const dealTrackerApiResponse = await fetch(
    `${t.context.api.tracker}/version`
  )
...
test('POST /', async (t) => {
  ...
  const { invocationConfig, connection } = await getClientConfig(
    new URL(t.context.api.aggregator)
  )
  ...
  const pieceOfferResponses = await Promise.all(
    pieces.map((p) =>
      Aggregator.pieceOffer(invocationConfig, p.link, group, { connection })
    )
  )
  ...
  console.log(
    'all pieces were successfully offered',
    pieces.map((p) => p.link)
  )
  ...
  const groupAggregates = await waitForStoreOperationOkResult(
    ...
    (
      /** @type {import("@ucanto/interface").Result<import("@storacha/filecoin-api/aggregator/api").AggregateRecord[], {}>} */ res
    ) => Boolean(res.ok?.length)
  )
  ...
  const aggregatedBuffers = await Promise.all(
    groupAggregates.ok.map((aggregate) =>
      waitForStoreOperationOkResult(
        () => t.context.store.aggregator.bufferStore.get(aggregate.buffer),
        (
          /** @type {import("@ucanto/interface").Result<import("@storacha/filecoin-api/aggregator/api").BufferRecord, {}>} */ res
        ) => Boolean(res.ok)
      )
    )
  )
  ...
  // @ts-ignore
  const entriesInAggregatedBuffers = aggregatedBuffers.reduce(
    (acc, aggregatedBuffer) => {
  ...
  const convergedEntry = entriesInAggregatedBuffers.find((aggregatedEntry) =>
    pieces.find((p) => p.link.equals(aggregatedEntry.piece))
  )
  t.truthy(convergedEntry)

  // Verify inclusion stored
  console.log('wait for inclusion records for pieces in aggregate ...')
  const inclusionRecords = await Promise.all(
    entriesInAggregatedBuffers.map((aggregateEntry) =>
      waitForStoreOperationOkResult(
        () =>
          t.context.store.aggregator.inclusionStore.get({
            piece: aggregateEntry.piece,
            aggregate: aggregateEntry.aggregate
          }),
        (res) => Boolean(res.ok)
      )
    )
  ...
  const offeredAggregatesForDeal = await Promise.all(
    groupAggregates.ok.map((aggregate) =>
      waitForStoreOperationOkResult(
        () =>
          t.context.store.dealer.aggregateStore.get({
  ...
  if (
    offeredAggregatesForDeal.find(
      (r) => r.ok?.status === 'accepted' || r.ok?.status === 'invalid'
    )
  ) {
  ...
  const offersForDeal = await Promise.all(
    offeredAggregatesForDeal.map((offeredAggregate) =>
      waitForStoreOperationOkResult(
        () =>
  ...
  for (const offer of offersForDeal) {
    const bufferAggregate = aggregatedBuffers.find((ab) =>
      ab.ok?.buffer.aggregate?.equals(offer.ok.value.aggregate)
    )

    for (let i = 0; i < offer.ok.value.pieces.length; i++) {
      t.truthy(
        bufferAggregate?.ok?.buffer.pieces[i].piece.equals(
          offer.ok.value.pieces[i]
        )
      )
    }
  }
  ...
  const acceptedAggregatesForDeal = await Promise.all(
    groupAggregates.ok.map((aggregate) =>
      waitForStoreOperationOkResult(
        () =>
          t.context.store.dealer.aggregateStore.get({
  ...
  const pieceOfferReceiptCid = pieceOfferResponses
    .find((p) => p.out.ok?.piece.equals(convergedEntry?.piece))
    ?.ran.link()
  ...
  console.log(
    `wait for piece/offer ${pieceOfferReceiptCid.toString()} receipt...`
  )
  ...
  console.log(
    `wait for piece/accept ${pieceAcceptReceiptCid.toString()} receipt...`
  )
  ...
  console.log(
    `wait for aggregate/offer ${aggregateOfferReceiptCid.toString()} receipt...`
  )
  ...
  if (!aggregateAcceptReceiptCid) {
    throw new Error(
      'aggregate/offer receipt has no effect for aggregate/accept'
    )
  }

  console.log(
    `wait for aggregate/accept ${aggregateAcceptReceiptCid.toString()} receipt...`
  )
...
/**
 *
 * @param {import('@storacha/filecoin-api/dealer/api').AggregateRecord} record
 */
